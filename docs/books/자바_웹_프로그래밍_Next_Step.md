...
### MVC 프레임워크 구현하기
 - 지금까지의 코드는 JSP에 많은 로직이 뒤섞여 있어 유지보수가 힘들다.
 - 이를 해결하기 위해 JSP에 집중되었던 로직을 모델, 뷰, 컨트롤러 3개의 역할로 분리해 개발하는 기법이 등장했다.
 - 이제부터는 JSP로 요청이 들어오지 않고 모든 요청이 컨트롤러로 들어온다. (DispatcherServlet)
 - DispatcherServlet은 요청 클라이언트의 요청을 받아 RequestMapping을 이용해 적절한 컨트롤러를 선택한다. 
 - 이처럼 각 컨트롤러의 앞에 모든 요청을 받아 각 컨트롤러에게 작업을 위임하는 방식을 프론트 컨트롤러 패턴이라고 한다.
...
### 프레임워크 VS 라이브러리
 - 프레임워크와 라이브러리는 중복 코드를 제거해 재사용성을 높여준다는 공통점이 있다.
 - 프레임워크는 특정 패턴 기반으로 개발하도록 강제한다.
 - 라이브러리는 강제하는 부분이 없다.
