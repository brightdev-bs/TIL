...
6. 최소 지식 원칙 : 객체는 상호작용하는 클래스의 개수와 상호작용 방식에 기울여야 한다. 
   - 시스템의 한 부분을 변경했을 때 다른 부분까지 줄줄이 고쳐야 하는 상황을 방지할 수 있다. 
7. 할리우드 원칙 : 저수준 모듈을 언제 어떻게 호출할지는 고수준 모듈에서 결정한다.
8. 어떤 클래스가 바뀌는 이유는 하나뿐이어야 한다.
   - 어떤 클래스에서 맡고 있는 모든 역할은 나중에 코드 변화를 불러온다.
   - 역할이 2개 이상 있으면 바뀔 수 있는 부분이 2개 이상이 된다.
...
### [퍼사드 패턴]
 - 서브시스템에 있는 일련의 인터페이스를 통합 인터페이스로 묶어 준다.
      - 영화를 보기 전에 팝콘 기계를 동작시켜 팝콘을 만들고, 스크린을 내리고, 빔 프로젝트를 키는 등의 모든 일련의 작업을 하나의 인터페이스로 묶는다.
      - 3개의 클래스를 하나의 클래스로 줄일 수 있다. 
...
### [반복자 패턴]
 ...
 - Iterator라는 인터페이스만 가지고 모든 컬렉션을 순회할 수 있다.
 - 자바의 Collection클래스는 모두 Iterable을 구현하고 있다.

### [컴포지트 패턴]
 - 객체를 트리구조로 구성해서 부분-전체 계층구조를 구현한다. 
     - 부분-전체 계층구조란 부분(메뉴와 세부메뉴) 들이 계층을 이루고 있지만 모든 부분을 묶어서 전체로 다룰 수 있는 구조를 말한다.
 - 클라이언트에서 개별 객체와 복합 객체를 똑같은 방법으로 다를 수 있게 해준다.
